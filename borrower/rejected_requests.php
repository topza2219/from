...
$page_title = "คำขอที่ถูกปฏิเสธ";
require_once dirname(__FILE__) . '/templates/header.php';
require_once dirname(__FILE__) . '/templates/sidebar.php';

try {
    // Fetch rejected requests with item details
    $query = "SELECT br.*, 
              i.item_name,
              i.description,
              i.category,
              ...
              DATE_FORMAT(br.rejected_at, '%d/%m/%Y %H:%i') as formatted_rejected_at,
    ...
    error_log('Error in rejected_requests.php: ' . $e->getMessage());
...
<div class="main-content p-6">
    <div class="stat-card">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-900">คำขอที่ถูกปฏิเสธ</h1>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="table-header">วันที่ต้องการยืม</th>
                            <th class="table-header">วันที่ต้องการคืน</th>
                            <th class="table-header">ผู้ปฏิเสธ</th>
                            <th class="table-header">เหตุผล</th>
                            <th class="table-header">วันที่ถูกปฏิเสธ</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($requests as $request): ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="table-cell">
                                    <?php echo $request['formatted_rejected_at']; ?>
                                </td>
                                <td class="table-cell">
                                    <button onclick="requestAgain(<?php echo $request['request_id']; ?>)" 
...
function requestAgain(requestId) {
    Swal.fire({
        title: 'ยืนยันการขอยืมใหม่',
        text: 'ต้องการสร้างคำขอยืมใหม่จากคำขอเดิมนี้ใช่หรือไม่?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonText: 'ใช่, ขอยืมใหม่',
        cancelButtonText: 'ยกเลิก'
    }).then((result) => {
        if (result.isConfirmed) {
            window.location.href = `request_again.php?id=${requestId}`;
        }
    });
}
</script>
